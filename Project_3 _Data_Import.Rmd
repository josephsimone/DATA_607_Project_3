...
### Tried a piechart (Not the best way to visualize data)
```{r}
#library(plotly)
#devtools::install_github("ropensci/plotly")

#top_10_tech_visual_pie <- data.frame("Keyword"=top_10_tech, top_10_tech)
#data <- top_10_tech_visual_pie[,c('Keyword', 'LinkedIn')]
#data

#plot_ly(data, labels = ~Keyword, values = ~LinkedIn, type = 'pie') %>%
  #layout(title = 'Top 10 Tech',
         #xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         #yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))

#plot_ly(data, type = 'bar') %>%
  #layout(title = 'top_10_tech'
         #)

#p <- plot_ly(data, labels = ~Keyword, values = ~LinkedIn, type = 'pie') %>%
  #layout(title = 'top_10_tech',
         #xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         #yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
...
s <- plot_ly(data, x = ~Keyword, y = ~LinkedIn, type = 'bar', name = "Linkedin") %>%
  ...
  add_trace(y = ~Monster, name = "Monster") %>% 
  add_trace(y = ~Total, name = "Average") %>% 
  layout(title = 'Technical Skills by Job Site', yaxis = list(title = '% of Data Science Jobs'), xaxis = list(title = 'Tech Skills'), barmode = 'group')
